...
logname = ('results/alpha=' + str(args.alpha) + '_'+ net.__class__.__name__ + '_' + args.name + '_'
           + str(args.seed) + 'CIFAR100_4p_new.csv')
...
def mixup_data(x, y, a=1.0, use_cuda=True):
    ...
    #print('index',index)
    #print('length', x.size())
    #print('length',len(x[index,:]), x[index,:].size())
    ...
    #print('y_a',y_a, y_a.size())
    #print('y_b',y_b, y_b.size())
    return mixed_x, y_a, y_b, y_c, y_d, lam


def mixup_criterion(criterion, pred, y_a, y_b, y_c, y_d, lam):
...
def train(epoch):
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        inputs, targets_a, targets_b, targets_c, targets_d, lam = mixup_data(inputs, targets,
                                                       args.alpha, use_cuda)
        inputs, targets_a, targets_b, targets_c, targets_d = map(Variable, (inputs,
                                                      targets_a, targets_b, targets_c, targets_d))
        outputs = net(inputs)
    
        loss = mixup_criterion(criterion, outputs, targets_a, targets_b, targets_c, targets_d, lam)
        train_loss += loss.item()
        _, predicted = torch.max(outputs.data, 1)
        total += targets.size(0)
        correct += (lam[0] * predicted.eq(targets_a.data).cpu().sum().float()
                    + lam[1] * predicted.eq(targets_b.data).cpu().sum().float()
                    + lam[2] * predicted.eq(targets_c.data).cpu().sum().float() + lam[3] * predicted.eq(targets_d.data).cpu().sum().float())
