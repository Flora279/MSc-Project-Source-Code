...
class ProbNNet4l(nn.Module):
    ...
    def __init__(self, rho_prior, prior_dist='gaussian', device='cuda', init_net=None, init_prior='random'):
        super().__init__()
...
class ProbNNet3l(nn.Module):
    ...
    def compute_kl(self,recompute=False):
        # KL as a sum of the KL for each individual layer
        if recompute:
            return self.l1.recompute_kl() + self.l2.recompute_kl() + self.l3.recompute_kl() 
        else:
...
def trainNNet(net, optimizer, epoch, train_loader, device='cuda', verbose=False):
    ...
    if verbose:
        print('trainNNet (ERM/Mixup prior training):',
...
def testNNet(net, test_loader, device='cuda', verbose=True):
    ...
    with torch.no_grad():
        for batch_id, (data, target) in enumerate(test_loader):
            ...
            total += target.size(0)
    if verbose:
        print('TestNNet, prior early stopping scheme,'
            f"-Prior: Test loss: {avgloss/(batch_id+1) :.5f}, Test err:  {1-(correct/total):.5f}")
    return avgloss/(batch_id+1), 1-(correct/total)


def trainPNNet(net, optimizer, pbobj, epoch, train_loader, lambda_var=None, optimizer_lambda=None, verbose=True):
    ...
    for batch_id, (data, target) in enumerate(train_loader):
        ...
        net.zero_grad()
        #use X-E loss to compute trainobj:
