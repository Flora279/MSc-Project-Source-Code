...
def loaddataset(name):
    ...
    if name == 'mnist':
        ...
        trainold = datasets.MNIST(
            'mnist-data/', train=True, download=True, transform=transform)
            
        #only use the first 25% of the train data 注意不能ranodm取，这样不同run跑的data都会不一样
        indices = np.arange(int(0.25 * len(trainold))).astype(int)
        train = torch.utils.data.Subset(trainold, indices)
        print('check the first data should be 5 across all experiments on MNIST', train[0][1])
        #still use whole test data
        test = datasets.MNIST(
        ...
        trainold = datasets.CIFAR10(
            './data', train=True, download=True, transform=transform)
            
        #only use 25% of the train data
        indices = np.arange(int(0.25 * len(trainold))).astype(int)
        train = torch.utils.data.Subset(trainold, indices)
...
def loadbatches(train, test, loader_kargs, batch_size, prior=False, perc_train=1.0, perc_prior=0.2, perc_val=0.0):
