...
prior_args = dict(
    ...
    epochs = 100,
    kl_penalty = 1,
    sigma = 0.01,  #0.01-0.05, important, better to tune for 0.01 and 0.03
    initialize_prior = 'random', #(or 'zeros') random
    dropout_prob = 0 #only used if deterministic nn, erm(dropout 0.2) vs mixup   erm 0 vs mixup
)
...
experiment_args = dict(
    ...
    perc_train = 1.0,    # select part of the data
    perc_val = 0.05,     #if perc_val>0, then we have validation for early stopping when training prior,only need val set for mixup/erm, not PAC-Bounds!!!! 
    perc_prior = 0.1,
    freq_test = 5,
...
Experiment Parameters
    ...
    delta : float
        confidence value(ÃŽÂ´) for the risk certificate training objective
    delta_test : float
        confidence value(ÃŽÂ´) for the risk certificate chernoff bound (used when 
        computing the risk)
    pmin : float
